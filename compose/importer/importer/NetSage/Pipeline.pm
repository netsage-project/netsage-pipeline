...
sub BUILD {
    ...
    my $grnoc_log = NetSage::Log->new( config => $self->logging_file );
    my $logger = NetSage::Log->get_logger();

    $self->_set_logger( $logger );

    # create and store config object
    my $config_obj = NetSage::myConfig->new( config_file => $self->config_file,
                                     force_array => 0 );
    ...
    if ( defined ( $self->shared_config_file ) ) {
        $shared_config_obj = NetSage::myConfig->new( config_file => $self->shared_config_file,
            force_array => 0 );
...
sub _rabbit_connect {
  ...
  while ( 1 ) {
    ...
    foreach my $direction ( @directions ) {
        ...
        $self->logger->info( "  RabbitMQ Settings: username: $rabbit_username, ssl: $rabbit_ssl " );
        ...
        try {
            ...
            if ( !defined $rabbit_host ) {
                $self->logger->error( "RabbitMQ host name not defined" );
                return;
            }
