...
NetSage::myConfig - The GRNOC Default Config parser
...
When asking for attributes denoted by '@' it will return only the attribute(s) without the hash
    ...
    my $foo = NetSage::myConfig->new(config_file => '/etc/db.xml', debug => 0, force_array => 0 schema => '/path/to/schema.csd')
...
=head2 new 
  Creates a new Config object
    ...
    my $config = NetSage::myConfig->new(config_file => $file, force_array => 0, debug => 0);

    or

    my $config = NetSage::myConfig->new( xml_string => $string);
