...
sub BUILD {
    ...
    my $config = NetSage::myConfig->new( config_file => $self->config_file,
                                     force_array => 0 );
...
sub _build_config {
    ...
    $self->logger->debug( 'Building NetSage::Config with config file ' . $self->config_file . '.' );

    return NetSage::myConfig->new( config_file => $self->config_file,
                               force_array => 0 );
}
...
sub _create_workers {
    ...
    #die "done";
        ...
        #my $worker = NetSage::FlowTagger->new( config => $self->config,
        #							      logger => $self->logger,
