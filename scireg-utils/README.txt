...
Current scripts/programs:
  - scireg2mmdb-input.py : converts main scireg.json to format needed by scireg2mmdb
  - scireg2mmdb.go : builds mmdb from JSON
  - dump_mmdb.go : dump entire mmdb for debugging
  - Makefile : builds go tools

To Do:
  - modify scireg2mmdb.go to append a single entry (or new script)
  - program to prompt use for all scireg fields, and add to both .json and .mmdb
